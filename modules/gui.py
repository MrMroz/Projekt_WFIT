...
class App(tk.Frame):
    def __init__(self, parent, grid_width = 40, grid_height = 30, *args, **kwargs):
        ...
        self.parent = parent # Okno nadrzędne

        # Wysokość i szerokość siatki komórek
        ...
        # Lista frame'ów (reprezentacji komórek)
        self.cell_widgets = []

        # Mechanika malowania komórek
    ...
    def set_up_button_frame(self) -> None:
        ...
        slider = tk.Scale(button_frame, from_=1, to=10, length=200 , orient='horizontal', variable=self.tick_rate)
    ...
    def update_gui_cells(self) -> None:
        for cell in self.board.changed_cells:
            ...
            if cell.state == 1: # Jeżeli żyje, kolor czarny, martwa - kolor biały
    ...
    def save_board_handler(self) -> None:
        filename = fd.asksaveasfilename(initialfile = 'zapis.json', defaultextension=".json", filetypes=[("All Files","*.*"),("JSON File","*.json")])
    ...
    def load_board_handler(self) -> None:
        ...
        if status == -1:
            tk.messagebox.showerror(title='Błąd wczytywania', message='Wymiary pliku nie pasują do wymiarów planszy')
        else:
            self.force_update_all_cells()
    
    def open_gh_page(self) -> None:
        os.system("start \"\" https://github.com/MrMroz/Projekt_WFIT")
