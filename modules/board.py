...
class Board:
    def __init__(self, width: int, height: int) -> None:
        ...
        self.height = height
        self.cells = []
        self.changed_cells = set() # Pamięć które komórki zmienią się/zmieniły się w kolejnym stanie
    ...
    def assign_neighbours(self):
        for y in range(self.height):
            for x in range(self.width):
                ...
                logging.debug(f"Komórka ({x},{y}) otrzymała referencje do {len(self.cells[y][x].adjacent_cells)} sąsiadów")
    ...
    def get_serialized_cells(self) -> list[int]: # Plansza w postaci listy obecnych stanów; do wypisywania w cli
        serialized = []
    ...
    def load_from_file(self, filename: str) -> int:
        ...
        if content['width'] == self.width and content['height'] == self.height:
            ...
            logging.warning(f'Niewłaściwe wymiary {content["width"]}x{content["height"]}, oczekiwano {self.width}x{self.height}')
            return -1